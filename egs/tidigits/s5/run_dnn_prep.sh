...
timit=/workspace/zhorai-workspace/datasets/tidigits_tr-c_te-c/data # @BUT

#echo "TEST LS: "
#ls local/timit_data_prep.sh

#local/timit_data_prep.sh $timit || exit 1
local/tr-c_te-c_tidigits_data_prep.sh $timit || exit 1
## TODO: using this script to create data/local/dict/lexiconp.txt, data/local/dict/nonsilence_phones.txt, etc. instead of using this:
##local/timit_prepare_dict.sh

local/tidigits_prepare_lang.sh  || exit 1;


## TODO: This is already done in the above tidigits_prepare_lang, so i'm commenting it out
...
## TODO: instead, we use tidigits_prepare_lang?
...
for x in train test; do
  steps/make_mfcc.sh --cmd "$train_cmd" --nj $feats_nj data/$x exp/make_mfcc/$x $mfccdir || exit 1;
