...
for x in train dev test; do
  mkdir -p data/$x
## TODO: there's already a wav.scp in data/local/wav.scp, so commenting out:
##  cp $srcdir/${x}_wav.scp data/$x/wav.scp || exit 1;
## TODO data/local/data/text was already created in local/tidigits_prepare_lang.sh, so comment out:
##  cp $srcdir/$x.text data/$x/text || exit 1;
## TODO: already a spk2utt in data/local/spk2utt AND a utt2spk etc. below
##  cp $srcdir/$x.spk2utt data/$x/spk2utt || exit 1;
##  cp $srcdir/$x.utt2spk data/$x/utt2spk || exit 1;
## TODO: I don't care about gender rn
##  utils/filter_scp.pl data/$x/spk2utt $srcdir/$x.spk2gender > data/$x/spk2gender || exit 1;
echo "stm: cp $srcdir/${x}.stm data/$x/stm"
echo "glm: cp $srcdir/${x}.glm data/$x/glm"
