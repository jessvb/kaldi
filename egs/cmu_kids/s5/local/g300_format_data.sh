...
. path.sh
  ...
  ndisambig=$[$ndisambig+1]; # add one disambig symbol for silence in lexicon FST.
	...
	cp data/local/oov.int data/lang
  ...
  word_disambig_symbol=`grep \#0 data/lang_test/words.txt | awk '{print $2}'`
	...
	for lm_suffix in tg; do
	 ...
	 # cat input/task.arpabo | arpa2fst - | fstprint |\
	 #    scripts/eps2disambig.pl | scripts/s2eps.pl |\
	 #    fstcompile --isymbols=$test/words.txt --osymbols=$test/words.txt --keep_isymbols=false --keep_osymbols=false | fstrmepsilon > $test/G.fst
	 ...
	 # fstisstochastic $test/G.fst
	  ...
	  cat input/task.arpabo | \
	    ...
	    grep -v '</s> </s>' | \
	 	arpa2fst - | fstprint |\
	     scripts/remove_oovs.pl ${DATA_LOCAL_DIR}/oovs.txt |\
	     scripts/eps2disambig.pl | scripts/s2eps.pl |\
	     fstcompile --isymbols=$test/words.txt --osymbols=$test/words.txt --keep_isymbols=false --keep_osymbols=false | fstrmepsilon > $test/G.fst
	  #cat input/G.txt | scripts/eps2disambig.pl | scripts/s2eps.pl | fstcompile --isymbols=$test/words.txt --osymbols=$test/words.txt --keep_isymbols=false --keep_osymbols=false | fstrmepsilon > $test/G.fst
	  fstisstochastic $test/G.fst
