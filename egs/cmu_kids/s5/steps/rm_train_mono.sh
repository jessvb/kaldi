...
compute-cmvn-stats  scp:$data/feats.scp ark:$dir/cmvn.ark 2>$dir/cmvn.log
if [ $? -ne 0 ]; then 
	echo "Run compute-cmvn-stats fail."
	echo "log: $dir/cmvn.log"
	exit 1
else 
	echo "compute-cmvn-stats succeed."
fi

feats="ark:apply-cmvn --norm-vars=false ark:$dir/cmvn.ark scp:$data/feats.scp ark:- | add-deltas ark:- ark:- |"

cat $data/text | sed "s/<s>//g" | sed "s/<\/s>//g" > text.tmp
# compute integer form of transcripts.
scripts/sym2int.pl --ignore-first-field $lang/words.txt < text.tmp > $dir/train.tra
if [ $? -ne 0 ]; then 
	echo "Run scripts/sym2int.pl fail."
	exit 1
else 
	echo "scripts/sys2int.pl succeed."
fi
rm text.tmp
...
gmm-init-mono "--train-feats=$feats subset-feats --n=10 ark:- ark:-|" $lang/topo 39  \
   $dir/0.mdl $dir/tree 2> $dir/init.log
if [ $? -ne 0 ]; then 
	echo "Run gmm-init-mono fail."
	exit 1
else 
	echo "gmm-init-mono succeed."
fi

echo "Compiling training graphs"
compile-train-graphs $dir/tree $dir/0.mdl  $lang/L.fst \
  ark:$dir/train.tra  "ark:|gzip -c >$dir/graphs.fsts.gz"  \
  2>$dir/compile_graphs.log
if [ $? -ne 0 ]; then 
	echo "Run compile-train-graphs fail."
	exit 1
else 
	echo "compile-train-graphs succeed."
fi
...
checkStatus align-equal-compiled:gmm-acc-stats-ali

# In the following steps, the --min-gaussian-occupancy=3 option is important, otherwise
# we fail to est "rare" phones and later on, they never align properly.

gmm-est --min-gaussian-occupancy=3  --mix-up=$numgauss \
    $dir/0.mdl $dir/0.acc $dir/1.mdl 2> $dir/update.0.log
checkStatus gmm-est
...
( cd $dir; rm final.mdl 2>/dev/null; ln -sf $x.mdl final.mdl; ln -sf $x.occs final.occs )
