...
for x in train test; do
  ...
  # TODO: added this for run_dnn.sh:
  # Prepare STM file for sclite:
  wav-to-duration --read-entire-file=true scp:data/$x/wav.scp ark,t:$delmedir/${x}_dur.ark || exit 1
  awk -v dur=$delmedir/${x}_dur.ark \
  'BEGIN{
  ...
  ' data/${x}/text >$delmedir/${x}.stm || exit 1
  ...
  # move stm and glm files to correct directories:
  mv $delmedir/${x}_dur.ark data/${x}/${x}_dur.ark # (also move the dur ark file, just in case ;P )
  mv $delmedir/${x}.stm data/${x}/stm
  mv $delmedir/${x}.glm data/${x}/glm

  # copy to nnet directory too!
  mkdir data-fmllr-tri3
  mkdir data-fmllr-tri3/${x}
  cp data/${x}/stm data-fmllr-tri3/${x}/stm
  cp data/${x}/glm data-fmllr-tri3/${x}/glm 
done
