...
if [ $stage -le 0 ]; then
  ...
  # dev
#  dir=$data_fmllr/dev ## NOTE: commenting out dev set for tidigits
#  steps/nnet/make_fmllr_feats.sh --nj 10 --cmd "$train_cmd" \
...
if [ $stage -le 2 ]; then
  ...
  $cuda_cmd $dir/log/train_nnet.log \
    ...
    $data_fmllr/train_tr90 $data_fmllr/train_cv10 data/lang_test_bg $ali $ali $dir || exit 1; ## NOTE: changed data/lang to data/lang_test_bg
  # Decode (reuse HCLG graph)
  steps/nnet/decode.sh --nj 20 --cmd "$decode_cmd" --acwt 0.2 \
    $gmmdir/graph $data_fmllr/test $dir/decode_test || exit 1;
  #steps/nnet/decode.sh --nj 20 --cmd "$decode_cmd" --acwt 0.2 \ ## NOTE: commenting out dev set for tidigits
...
if [ $stage -le 3 ]; then
  ...
  steps/nnet/align.sh --nj 20 --cmd "$train_cmd" \
    $data_fmllr/train data/lang_test_bg $srcdir ${srcdir}_ali || exit 1;
  steps/nnet/make_denlats.sh --nj 20 --cmd "$decode_cmd" --acwt $acwt \
    --lattice-beam 10.0 --beam 18.0 \
    $data_fmllr/train data/lang_test_bg $srcdir ${srcdir}_denlats || exit 1;
fi
...
if [ $stage -le 4 ]; then
  ...
  steps/nnet/train_mpe.sh --cmd "$cuda_cmd" --num-iters 6 --acwt $acwt \
    --do-smbr true \
    $data_fmllr/train data/lang_test_bg $srcdir ${srcdir}_ali ${srcdir}_denlats $dir || exit 1
  # Decode
  for ITER in 1 6; do
    ...
    #steps/nnet/decode.sh --nj 20 --cmd "$decode_cmd" \
    #  --nnet $dir/${ITER}.nnet --acwt $acwt \
    #  $gmmdir/graph $data_fmllr/dev $dir/decode_dev_it${ITER} || exit 1 ## NOTE: commenting out dev set for tidigits
  done 
fi
